...
ALLOW_PLUGIN_LOADING = os.environ.get('ALLOW_PLUGIN_LOADING') == '1'
...
def create_app():
    ...
    def upload_document():
        ...
        size = stored_path.stat().st_size
                ...
                # Verify PDF header
                with open(stored_path, "rb") as _fh:
                    _head = _fh.read(5)
                if _head != b"%PDF-":
                    try:
    ...
    def delete_document(document_id: int | None = None):
        ...
        try:
            with get_engine().connect() as conn:
                query = text("SELECT id, path FROM Documents WHERE id = :id AND ownerid = :uid")
    ...
    def create_watermark(document_id: int | None = None):
        ...
        try:
            with get_engine().connect() as conn:
                row = conn.execute(
                    text("""
                        SELECT id, name, path FROM Documents WHERE id = :id AND ownerid = :uid AND ownerid = :uid
                        LIMIT 1
                    """),
                    {"id": doc_id},
                ).first()
    ...
    def load_plugin():
        """
        Load a serialized Python class implementing WatermarkingMethod (disabled by default).
        """
        if not ALLOW_PLUGIN_LOADING:
